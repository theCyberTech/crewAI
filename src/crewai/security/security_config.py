...
class SecurityConfig(BaseModel):
    ...
    - Optional encryption for inter-agent communication.
    ...
    Attributes:
        ...
        encrypt_communication (bool): Flag to enable or disable encryption for data passed
            between agents (e.g., task outputs that become context for the next agent).
            Defaults to False.
        encryption_key (Optional[bytes]): The symmetric encryption key used for encrypting
            and decrypting communication if `encrypt_communication` is True. If not provided
            and encryption is enabled, a new key will be auto-generated. This key is
            excluded from serialization (e.g., `to_dict()`) for security.

    Security Note:
        The `encryption_key` for `encrypt_communication` is auto-generated by default
        if not provided when encryption is enabled. This simplified key management is
        intended for ease of use. For production deployments requiring strong security,
        users should consider implementing more robust key management strategies,
        such as securely providing and managing their own keys.
    """
    ...
    encrypt_communication: bool = Field(
        default=False,
        description="Enable encryption for agent-to-agent communication."
    )

    encryption_key: Optional[bytes] = Field(
        default=None,
        description="Encryption key for communication. Auto-generated if not provided and encryption is enabled.",
        exclude=True
    )
    ...
    def generate_encryption_key_if_needed(self) -> 'SecurityConfig':
        """Generate encryption key if encryption is enabled and no key is provided."""
        if self.encrypt_communication and self.encryption_key is None:
            self.encryption_key = os.urandom(32) # Generate a 256-bit key
        return self
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        # encryption_key is excluded by default due to exclude=True in Field definition
        return result
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'SecurityConfig':
        ...
        encrypt_communication = data_copy.pop("encrypt_communication", False)

        # encryption_key is not expected in from_dict for security reasons
        # If it needs to be set, it should be done explicitly after instantiation
        # or through a secure mechanism.
